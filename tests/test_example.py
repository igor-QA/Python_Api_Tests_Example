...
class TestExample:
    ...
    def test_create_new_booking(self, client): #Проверка существования созданного бронирования;
    ...
    def test_new_booking_exists(self, client): #Проверка работы обновления данных бронирования;
        data = random.random_booking()
        res = client.vr(client.create_booking(data), [200, 201])
        created = res.json()
    ...
    def test_update_booking(self, client): #Запрос рандомного ID бронирования с сервера;
        data = random.random_booking()
        res = client.vr(client.create_booking(data), [200, 201])
        created = res.json()
        bookingid = created.get("bookingid")
